...
int main() {
    ...
    head = malloc(sizeof(struct packetList));
    struct packetList* temp = malloc(sizeof(struct packetList));
    head->next = temp;
    temp->protocol = 80;
    head->protocol = 1;
    ...
    printf("%d\n", head->next->protocol);
    while (1) {
        printf("1. HTTP  2. DNS  3. TELNET  4. FTP\n");
    ...
    struct packetList* curr = head->next;
    while (curr != NULL)
    {
        printf("%d\n", curr->protocol);
        curr = curr->next;
...
void ProcessPacket(unsigned char* buffer, int size, int num)
{
    ...
    //struct packetList* iter = save->next;
    //struct packetList* packet = malloc(sizeof(struct packetList));
 ...
 */
    ...
    case 3:
        ...
        if (ntohs(tcph->dest) == 23 || ntohs(tcph->source) == 23)
            print_telnet(buffer, size);
        break;

    case 4:
        //filtering FTP   
        if (ntohs(tcph->dest) == 21 || ntohs(tcph->source) == 21)
            print_ftp(buffer, size);
        break;
    }
    //printf("processpacket exit\n");
}
...
void print_telnet(unsigned char* Buffer, int Size) {
    int etherlen = sizeof(struct ethhdr);
    struct iphdr* iph = (struct iphdr*)(Buffer + sizeof(struct ethhdr));
    unsigned short iphdrlen = iph->ihl * 4;
    struct tcphdr* tcph = (struct tcphdr*)(Buffer + iphdrlen + sizeof(struct ethhdr));
    ...
    if (Size - header_size > 0) {
        if (Size == 60 && iphdrlen + tcph->doff * 4 == ntohs(iph->tot_len))
            ...
            printdata(Buffer + header_size, ntohs(iph->tot_len) - iphdrlen - tcph->doff * 4);
        }
        else {
            printf("TELNET Header\n");
...
void print_ftp(unsigned char* Buffer, int Size) {
    int etherlen = sizeof(struct ethhdr);
    struct iphdr* iph = (struct iphdr*)(Buffer + sizeof(struct ethhdr));
    unsigned short iphdrlen = iph->ihl * 4;
    struct tcphdr* tcph = (struct tcphdr*)(Buffer + iphdrlen + sizeof(struct ethhdr));
