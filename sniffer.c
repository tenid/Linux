...
void simply_print(unsigned char* , int );
...
int main() {
	...
	while (1) {
		printf("1. HTTP  2. DNS  3.ICMP   4.ALL\n");
...
void ProcessPacket(unsigned char* buffer, int size, int num)
{
	...
	case 4:
		if(check==0)
	        	printf("Source\t\t\t Destination\t\t Protocol\t Length\t Info\n");
		check=1;
...
void simply_print(unsigned char* Buffer, int size){
        struct iphdr *iph = (struct iphdr *)(Buffer + sizeof(struct ethhdr));
	memset(&source, 0, sizeof(source));
        source.sin_addr.s_addr = iph->saddr;
	...
	//printf("Source\t Destination\t Protocol\t Length\t Info\n");
	printf("%-20s\t %-20s\t %-10d\t %-10d\t \n", inet_ntoa(source.sin_addr),inet_ntoa(dest.sin_addr),iph->protocol, ntohs(iph->tot_len));
